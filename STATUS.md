...
> 763 tests across 25 groups, all green (+ 246-case djot-ref gate,
> 225 baselined). Phase 4 typed-by-construction
> view constructors landed (`TEAWeb.Html.Typed` — Step 4); a mounted
> view's accessibility is now provable at the boundary
> (`TEAWeb.Html.Accessible` — `AccessibleView`/`mountAccessible` carry
> `IsValidHtml × StructuralAA` witnesses). AA catalog
> expanded (plan §P3.2) from 12 → 70 rows: 27 Structural (executable +
> type-promoted + strict in elaboration), 19 Heuristic, 24 Runtime —
> covering 35 distinct WCAG 2.1 A/AA success criteria. 16 rows are
> vendored from the W3C ACT-rules corpus; the Runtime rows document the
> undecidable AA landscape (contrast, reflow, focus-visible, keyboard,
> captions, status-messages, …) and the P3.3 partition audit proves they
> stay out of the type layer. **Pass.idr data-interpreter refactor landed (plan §P3.2)**:
> `Cribrum.AA.Pass.nodeRuleImpls` is a `List (Rule, NodeRuleCheck)`
> registry the per-node traversal folds over (27 rows); `treeRuleImpls`
...
> regressions fail the gate, additions are tracked. **225 / 246**
> upstream-ingested tests now pass (full `jgm/djot` corpus ingestion
> shipped). The latest round added code-fence-info rejection
> (code-blocks-005), def-list code bodies (definition-lists-004), and
> progressive-indent sublist nesting (lists-003), then un-baselined
> headings-005 (`##` → empty heading, now rejected by the strict
> `no-empty-heading` rule — a deliberate divergence) for a 223 → 225 net.
> The prior climb (212 → 223) added bracket-balanced
...
> (blockquote-012). The climb (205 -> 212) replaced the first-closer
...
>
> **Step 13 heading → section inference** (plan §1b): `elaborate` now
> wraps heading-level sequences in nested `<section>` landmarks — a
> heading at level L opens a section holding it plus following blocks
> until the next heading of level <= L; deeper headings nest. A
> heading's explicit `{#id}` moves onto its section. The parser gained
> **lazy heading continuation**: a heading-marker line followed by plain
> or same-level-marker lines folds into one heading (corpus
> `headings-003`/`-007`/`-008`); a different-level marker splits into
> sibling headings (`headings-006`). `Cribrum.Pipeline.Anchor` was
> refactored — `slugify` → `autoId` (the Djot reference identifier:
> case-preserving, `_` kept, `-N` disambiguation) and
> `addHeadingIds` → `addSectionIds` (auto-ids land on `<section>`s, not
> headings); `harvestHeadings` reads section ids. The docsnav demo
> migrated to the new case-preserving section anchors. Net djotref
> movement: 101 -> 113 / 246 (12 `headings-*`/`attributes-*` tests).
...
| `Cribrum.AA.Catalog`      | 3+4   | Shared rule catalog (single source of truth across pass + future types). Types (`Rule`/`Confidence`/`Severity`) in `Cribrum.AA.Catalog.Types`; rule data in `Cribrum.AA.Catalog.Generated`, ingested from `ingest/aa.ts` by `ingest/aa-catalog.ts` (plan §P3.1 scaffold) with `make ingest-check` drift gate. **70 rows** (35 distinct WCAG SCs): **27 Structural** (executable + type-promoted + strict-enforced) — img-alt, anchor-href, heading-no-skip, document-lang, iframe-title, label-for-control, fieldset-legend, link-name, button-name, duplicate-id, unique-main, area-alt, link-empty-href, meta-no-refresh, summary-not-empty, track-kind, input-image-alt, object-name, th-scope-valid, th-has-name, no-empty-heading, select-has-options, caption-first-child, input-button-name, aria-hidden-body, aria-role-valid, autocomplete-valid; **19 Heuristic** (pass-only warnings incl. alt-meaningful, aria-label-redundant, positive-tabindex + 16 vendored ACT-corpus rows, `source="act"`); **24 Runtime** (catalog metadata for undecidable AA SCs — contrast 1.4.3, resize 1.4.4, reflow 1.4.10, non-text-contrast 1.4.11, text-spacing 1.4.12, keyboard 2.1.1, focus-visible 2.4.7, label-in-name 2.5.3, captions/audio-desc, error-prevention, status-messages 4.1.3, …; no impl, partition-audited as unreachable from the type layer). ACT rows pull from `ingest/act-rules/_rules/*.md` (pinned commit) via `ingest/act-rules.ts`. |
| `Cribrum.AA.Pass`         | 3     | **Data interpreter** (plan §P3.2): per-node traversal folds over `nodeRuleImpls : List (Rule, NodeRuleCheck)` — 27 rows pairing each catalog `Rule` with its impl through five tiny adapters (`pAttr`/`pNode`/`pCh`/`pAttrsOnly`/`pRoot`) that lift per-rule signatures to a single `Path -> Bool -> HExpr -> List Finding` shape. `treeRuleImpls` mirrors the same data view for the three whole-tree rules (heading-no-skip, duplicate-id, unique-main); `checkAA` keeps them as explicit `++` terms for mutation-gate stability. Total; confidence-partitioned (`structuralFindings` / `heuristicFindings`). Adding a per-node rule = one row. |
| `Cribrum.AA.Typed`        | 4     | **All 27 Structural rules** from the catalog promoted to type-level propositions via `So`: img-alt, anchor-href, iframe-title, label-for-control, fieldset-legend, button-name, link-name, area-alt, link-empty-href, meta-no-refresh, summary-not-empty, track-kind, input-image-alt, object-name, th-scope-valid, th-has-name, no-empty-heading, select-has-options, caption-first-child, input-button-name, aria-hidden-body, aria-role-valid, autocomplete-valid (per-node `All` over `walkNodes`); document-lang (root-only); heading-no-skip, duplicate-id, unique-main (whole-tree bool + `So`). Decision via `decSo` (and `All` for per-node rules). Each per-node rule's typed wrapper is a 5-line alias through `Cribrum.AA.Promote`. Partitioning witness `isTypedPromoted : String -> Bool` exposes the promoted-id set; `Test.Cribrum.AA.Partition` (plan §P3.3) asserts it agrees with `confidence == Structural` across `allRules`. `Cribrum.Elaborate.StructuralAA` is now the 27-tuple strict conjunct — **every** Structural catalog rule is enforced by construction at elaboration (the round-2 forms/tables/aria rules were promoted alongside the originals). `decStructuralAA` short-circuits to `(ruleId, path)` on first failing predicate. |
...
| `TEAWeb.Html.Typed`       | T1    | **Phase-2 typed-by-construction view-builder** (plan §T1 follow-on). 40 element constructors (`pT_`/`ulT_`/`liT_`/... plus the table family `tableT_`/`captionT_`/`colgroupT_`/`theadT_`/`tbodyT_`/`tfootT_`/`trT_`/`tdT_`/`thT_`/`colT_`) return `TypedView <tag> msg`; child positions consume `Child <parent> msg` (`c_`/`tx_`/`cm_`) carrying a compile-time `So (isTagAllowedIn parent child)` witness — a child the parent's content model forbids fails to typecheck. Catalog parity gated by `pddt_typed_predicate_matches_catalog` (general) + `pddt_typed_table_predicate_matches_catalog` (9 table parents × 18 children), both cross-validated against `Cribrum.Html.Valid.childAllowedBool` so the allowed-child sets are never written twice. Bridges to the untyped layer via `unTyped : TypedView tag msg -> View msg` for callers (e.g. `TEAWeb.Program.view`). |
...
| `TEAWeb.Sub`              | T4    | **Leaves shipped**: `None`/`Batch` + `OnKeyDown` / `OnKeyUp` / `OnAnimationFrame` / `Every` / `Port`. `flatten` strips `None` / `Batch` to leaf list; `subCallbackId` exposes per-leaf cbId. **Sub-diff core (D4)**: `subKey` keys each leaf by its structural identity (cbId + reinstall-forcing params — `Every`'s period, `Port`'s name; projections excluded); `diffSubs prev next` returns `(toInstall, toTeardown-by-cbId)` so the runtime add/removes only changed listeners across renders. Pure + chez-tested (`Test.TEAWeb.SubDiff`). `OnResize` deferred. Runtime install lives in `TEAWeb.Runtime`. |
| `TEAWeb.Runtime`          | T3+T4 | **MVP runtime + Sub-leaf install**: `mount` + tail-recursive interpreter loop in Idris; `installDispatch` installs single global `window.__cribrumDispatch`; `runCmd` interprets Focus/Blur via FFI; reconcile after each update keeps state ref + handler table in lockstep. `installSubs` walks the initial `Sub msg` tree at mount, calls the right installer per leaf (`installSubKeyDown` / `installSubAnimationFrame` / `installSubInterval` / `installSubPort`), and stashes a `(cbId, Event -> IO msg)` projection that reads the right window slot (`__cribrumKey` / `__cribrumTimestamp` / `__cribrumPortMsg`). `RuntimeState.subHandlers` persists across renders so Sub-driven deliveries survive view-handler refresh. **Sub-tree diff landed (D4)**: after each `update` the dispatcher re-evaluates `subscriptions newModel`, runs `diffSubs` against `RuntimeState.prevSubs`, tears down vanished leaves (new `teardownSub` FFI: `removeEventListener` / `clearInterval` / `cancelAnimationFrame`+cancelled-flag / port-slot delete, via a per-cbId `window.__cribrumSubHandles` registry the installers now populate) then installs newly-appeared ones, and rebuilds `subHandlers` from the next leaf set (`installSubsLeaves` FFI-install + pure `leafEntry`/`leafEntries` projection split). Teardown runs before install so a same-cbId param change (e.g. `Every` period) clears the old resource first. JS-backend execution only; chez type-checks via multi-spec %foreign with a scheme: fallback. |
...
+ 96 Elaborate + 19 Render.Html + 1 Render.Dom + 87 AA.Pass + 88 AA.Typed
...
640 Cribrum. 10 TEAWeb.Html + 13 TEAWeb.Html.Typed + 8 TEAWeb.Html.Accessible
+ 10 TEAWeb.Event + 10 TEAWeb.Cmd + 16 TEAWeb.Sub + 13 TEAWeb.SubDiff
+ 6 TEAWeb.Ports + 9 TEAWeb.Keyed + 19 TEAWeb.AttrDiff + 9 TEAWeb.Program
= 123 TEAWeb. Plus the djot-ref reference-suite gate (`tools/run-djotref/`,
246 corpus tests against a 225-test baseline; separate harness from the
in-suite tests). **Total: 763 in-suite tests, plus 246 djot-ref tests (225
passing, 21 expected-fail under baseline).**
...
## Mutation gate (222 mutants, 0 surviving)
