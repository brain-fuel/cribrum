...
- **Phase 4**: all 10 Structural AA rules in the catalog are now promoted
  ...
  skip, duplicate-id). Still ahead: the `Cribrum.AA.Promote` factor that
  compresses the per-rule boilerplate (plan §P4.2), and the sharpening
  of `Elaborate`'s `StructuralAA` codomain from unit to the actual
  conjunct of these 10 propositions.
